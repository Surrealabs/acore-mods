// wrathguard.cpp — Wrathguard AI & CreatureScript
// Combat AI only. Summoning, spell interactions, stat sync, and
// formation are handled by guardian_manager.cpp.

#include "warlock_expanded.h"

using namespace WarlockExpanded;

namespace
{
    constexpr uint32 REND_CD_MS            = 15000;
    constexpr uint32 HEMORRHAGE_CD_MS      = 10000;
}

// ─── Wrathguard combat AI ──────────────────────────────────────────────────────
struct wrathguard_ai : public CreatureAI
{
    explicit wrathguard_ai(Creature* creature)
        : CreatureAI(creature),
          rendTimer(1000), hemorrhageTimer(3000),
          followingOwner(true) { }

    uint32 rendTimer;
    uint32 hemorrhageTimer;
    bool followingOwner;

    void Reset() override
    {
        rendTimer        = 1000;
        hemorrhageTimer  = 3000;
    }

    Player* OwnerPlayer() { return GetOwnerPlayer(me); }

    void UpdateAI(uint32 diff) override
    {
        Player* owner = OwnerPlayer();
        if (!owner || !owner->IsAlive())
        {
            if (me->GetVictim())
            {
                me->AttackStop();
                me->GetMotionMaster()->Clear();
            }
            return;
        }

        // Read shared combat state (written by guardian_manager)
        Unit* desiredTarget = ResolveOwnerTarget(owner);
        bool ownerIdle = IsOwnerIdle(owner);

        // If owner idle or no target, drop combat and follow
        if (ownerIdle || !desiredTarget)
        {
            if (me->GetVictim())
            {
                me->AttackStop();
                me->CombatStop(true);
                me->GetMotionMaster()->Clear();
            }
            if (!followingOwner)
            {
                float angle = GetGuardianFollowAngle(me, owner);
                me->GetMotionMaster()->MoveFollow(owner, MELEE_FOLLOW_DIST, angle);
                followingOwner = true;
            }
            return;
        }

        // Leash check
        if (me->GetDistance(owner) > LEASH_RANGE)
        {
            if (me->GetVictim())
            {
                me->AttackStop();
                me->CombatStop(true);
                me->GetMotionMaster()->Clear();
            }
            float angle = GetGuardianFollowAngle(me, owner);
            me->GetMotionMaster()->MoveFollow(owner, MELEE_FOLLOW_DIST, angle);
            followingOwner = true;
            return;
        }

        // Engage desired target
        if (me->GetVictim() != desiredTarget)
        {
            me->AttackStop();
            me->GetMotionMaster()->Clear();
            AttackStart(desiredTarget);
            followingOwner = false;
        }

        // Charge if far from target
        if (me->GetDistance(desiredTarget) > 15.0f)
            me->CastSpell(desiredTarget, SPELL_CHARGE, true);

        if (!me->GetVictim()) return;
        Unit* victim = me->GetVictim();

        // ── DoT rotation (filler/heavy/special triggered by guardian_manager) ──
        if (rendTimer <= diff)
        {
            if (victim && victim->IsAlive() && !victim->HasAura(SPELL_REND, me->GetGUID()))
                me->CastSpell(victim, SPELL_REND, true);
            rendTimer = REND_CD_MS;
        }
        else rendTimer -= diff;

        if (hemorrhageTimer <= diff)
        {
            if (victim && victim->IsAlive())
                me->CastSpell(victim, SPELL_HEMORRHAGE, true);
            hemorrhageTimer = HEMORRHAGE_CD_MS;
        }
        else hemorrhageTimer -= diff;

        DoMeleeAttackIfReady();
    }

    void OnCharmed(bool /*apply*/) override { }
};

// ─── CreatureScript ────────────────────────────────────────────────────────────
class WrathguardCreatureScript : public CreatureScript
{
public:
    WrathguardCreatureScript() : CreatureScript("WrathguardCreatureScript") { }

    CreatureAI* GetAI(Creature* creature) const override
    {
        if (creature->GetEntry() == WRATHGUARD_ENTRY)
            return new wrathguard_ai(creature);
        return nullptr;
    }
};

void AddSC_Wrathguard()
{
    new WrathguardCreatureScript();
}
